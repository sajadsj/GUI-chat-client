...
void data_thread(transtruct tmp, SOCKET current_client, client_class& clients_class, mutex& m)
{
	...
	while (true)
	{
		...
		if (chat_reqs != "")
		{
            ...
            strcpy_s(tmp.chat_req, chat_reqs.c_str());
		...
		if (msg_for_me != "")
		{
            ...
            strcpy_s(tmp.msg, msg_for_me.c_str());
			int send_r = send(current_client, reinterpret_cast <char*> (&tmp), sizeof(tmp), 0);

            cout << " send_r is " << send_r << endl << flush;


                cout << "\n data threade: " << current_username <<"sent: "<< msg_for_me << endl;
                if (send_r != SOCKET_ERROR)
				{
					m.lock();
					//delet the msg
		...
		else
		{
			...
			case 1:
				...
				if (clients_class.search(tmp.username, tmp.pass) == true && clients_class.on_or_not(tmp.username) == true)
				{
					...
					else
					{
                        ...
                        //tmp.onlines = clients_class.get_onlines(tmp.username);
                        online_string = clients_class.get_onlines(tmp.username);
                        strcpy_s(tmp.onlines, online_string.c_str());
						//welcomeMsg = "Hellow " + tmp.username + ", welcome to chat server!\nOnlines are:\n" + online_string;
					}
			...
			case 3:
				...
				if (online_refresh != "")
				{
                    ...
                    strcpy_s(tmp.onlines, online_refresh.c_str());
				}
				else
				{
			...
			case 5:
                    msg = "";
                    msg = string(tmp.username) + " : " + string(tmp.msg);
                    ...
                    {
                        //send to tmp.username : you can not send msg to tmp.targetuser
